...
3. Call `list_resume_sources` and preserve every returned hash in `sourceResumeHashes`. A replacement resume is an additional source, not permission to overwrite prior evidence; re-run extraction whenever status returns `extract_accomplishments`.
