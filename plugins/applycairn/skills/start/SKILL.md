...
Keep the user-facing journey to four steps: (1) install and type `start`, (2) choose the reason for coming and capture one work moment (a resume is optional), (3) answer short Career Memory questions, and (4) review results and download the needed Markdown, DOCX, PDF, or complete workspace ZIP.
...
4. Route by intent. For documenting, review/promotion, growth, or transition, start with `capture_career_event`, clarify the story, and use `confirm_career_event` when the user confirms a complete resume-optional Career Story. For active search, offer resume extraction when available, then `$intake-interview`, `$build-master-resume`, `$review-career-memory`, or `$daily-run`.
